...
- **Lifecycle:** parent-managed CRUD, seeded with a fixed default set of **four** — Easy, Medium, Hard, Very Hard — each with its own paired Reward Category; extensible beyond that by the parent.
- **Rules:**
  ...
  - **`tierId` and `categoryId` are opaque, sequential, minted-in-pairs IDs** — `D01`/`R01`, `D02`/`R02`, … — never derived from `label` (which FR-3 lets the parent rename freely). The **seeded four** are fixed on every install: Easy `D01`/`R01`, Medium `D02`/`R02`, Hard `D03`/`R03`, **Very Hard `D04`/`R04`**. Numbers are **never reused** after a delete; the mint counter is persisted (`meta.nextSeq`), not recomputed from surviving rows.
...
- **Required fields:** `id`, `name`, `curriculumId`, `courseCode` (short, human-readable, e.g. `SAXMATH5` — keeps generated Activity IDs readable; **unique across all Course templates, case-insensitively**; alphanumeric only; never `CHR`, `EVT`, or `TPL`), `mainCategory` (`school` — Chore and Family Event are separate entities and never appear here), `state` (`template` | `instance`), and for instances `sourceTemplateId` + `childId` + `instanceToken` (see Rules below).
  - **There is no `lessons[]` field.** A Course's Lessons are a query — `lessons` filtered by `courseId`, sorted by `order` — not an embedded array. An array *and* a foreign key would be two sources of truth for the same relationship, and the one that silently drifts (a Lesson present in the store but missing from its Course's array) is invisible in every UI and fatal to pacing. §2.8 drops `activities[]` for the identical reason.
  - **`instanceToken`** (Instance-only): the random token minted for this Instance at stamping and baked into every one of its Activity IDs (§2.5). It is **stored on this record** precisely so that adding a new Activity to an existing Instance (Mgmt SRS 04 FR-10) can mint an ID carrying the right token **without parsing an existing Activity ID** — no code, anywhere, on either side, ever parses an ID to recover it (Interchange Contract §4).
...
- **Rules:** an Instance never links back to the template for content — it is a full copy. `sourceTemplateId` is provenance/label only, **not** a live reference. No propagation (§5.1). **A diverged Instance is never promoted back into the template library** — there is no archive-as-template path; a parent wanting to reuse an instance's content as a new template re-authors it manually. **An Instance's own Course-level fields are editable exactly as its Lessons are** (Mgmt SRS 04 FR-13): `name` and every optional field are freely editable, and edits never touch the template. `courseCode` is the sole exception, and it is frozen by construction rather than by a special rule — a stamped Instance has Activities beneath it from its first instant, so §2.4's freeze applies immediately and permanently. `mainCategory`, `sourceTemplateId`, `childId`, and `instanceToken` are never editable by any path. **An Instance stores no pacing pointer.** How far its content has been walked is *derived* from the **Generation Log** (§2.10a), never stored on the Instance: an Activity is **already sent** when a `sent` log row carries its `id`, and **pending** otherwise. Pacing-walk order — Lessons in `Lesson.order`, and within each Lesson, Activities in **`Activity.order`** ascending (§2.5; Mgmt SRS 05 §2.4) — orders only the *pending remainder* for the next run. Because membership is keyed by `id`, reordering, deferring, or pulling an Activity out of sequence cannot corrupt a walk position — there is no single high-water mark to fall out of step. A freshly-stamped Instance has an empty log: every Activity is pending.
  - **`courseCode` freezes once any Activity exists beneath the Course** (in any of its Lessons). Editing it afterward would strand every already-minted Activity ID, and would let a later Course take the freed code and mint a colliding one — the random `instanceToken` used to absorb this, but a template ID's literal `TPL` does not. The edit form disables the field with a reason; it does not silently ignore the change. **Deleting a Course outright *does* free its `courseCode` for reuse, and that is safe** — its template Activities are gone from the store, and any Instances stamped from it carry random tokens.
  - Uniqueness is scoped to `state: template` only. Instances inherit `courseCode` by copy, and two stamps of one template legitimately share it; the `instanceToken` is what keeps their Activity IDs apart.
  - **A stamped Instance's Lessons and Activities are editable** (Mgmt SRS 04) — this is what "the instance can diverge from the template" means in practice. Edits never propagate back to the template, and never touch it. Two divergence hazards are inherent and warned about rather than blocked: editing or deleting an Activity the child has **already received** changes nothing on their device (one-way interchange, §5.1), and deleting one they have **already completed** turns their CSV row into an unmatched row at import (§4.3).
...
- **Required fields:** `id` (stable interchange join key — see ID scheme below), `activityType` (reference to §2.5a), `title`, `required` (bool), `payload` (type-specific — a selector reference for platform-hosted content, never a URL), `difficultyTier` (references §2.3), `order` (integer — position within the owning Lesson; contiguous from `0`, system-maintained, reorderable by the parent; **this is the pacing walk order**, Mgmt SRS 05 §2.4).
...
- **ID scheme:** a readable composite, `{courseCode}-{instanceToken}-{lessonCode}-{seq}`, e.g. `SAXMATH5-f3k9-L03-02`. Four segments, always.
  - **Template Activities use the same grammar with a reserved literal token in segment 2:** `{courseCode}-TPL-{lessonCode}-{seq}`, e.g. `SAXMATH5-TPL-L03-02`. A template ID is structurally distinguishable from a stampable one by eye and by assertion, and **never crosses the interchange**.
  - `instanceToken` — a short **random** base36 token minted once per stamp event, shared by every Activity in that instance, **uniqueness-checked at mint and re-rolled on collision**, and never equal to `tpl`/`chr`/`evt` (case-insensitively). This is the collision firewall: two stamps of the same template produce two different tokens. It encodes nothing — in particular, not `childId` (§2.1).
  - `seq` — a **persisted, never-reused** per-Lesson counter, held on the Lesson record as `nextActivitySeq` (§2.8) and advanced in the same transaction as the Activity write. **It is never recomputed from the Activities that currently exist.** Deleting an Activity does not free its number: a `max(existing) + 1` implementation reuses it, and in an instance that means minting the completion-CSV join key of work the child has *already completed and exported*. Each Lesson — template or instance — carries its own counter. A Lesson created from scratch (template authoring, or Mgmt SRS 04 FR-9's instance-Lesson creation) seeds it to `1`.
  - **At stamp, `seq` values are copied, not re-minted.** Each Instance Activity keeps the exact `seq` its template original carried, and the Instance Lesson's `nextActivitySeq` is **copied forward from the template Lesson's current value** rather than reset. An Instance Activity's ID therefore differs from its template original **in segment 2 alone** — which is what makes a template↔instance diff checkable by eye, and what makes "did the stamp regenerate every ID?" a one-glance question. This is safe because the `instanceToken` already guarantees disjointness: two Instances of one template, and the template itself, can hold the same `seq` in the same `lessonCode` and still never collide.
  - `seq` is **not** `sequenceNumber` and **not** `order`. See the three-numbers note below.
...
- **Three numbers live on an Activity, and a build session will wire the wrong one:**
  ...
  **Reordering an Activity changes `order` only.** It never touches `seq` or `sequenceNumber`.
...
- **Seed data (10 canonical types):**
  ...
  | `activityTypeKey` | Type (`label`) | capturePattern | structurePattern |
  |---|---|---|---|
  ...
  *(`activityTypeKey` values locked at M5 — TDS_Slice_M5_Management_App_Rev7 §1a, Management SRS Module 12 §4. Kebab-case for the two multi-word keys; the other eight are single lowercase words. These are permanent identifiers — `label` is the freely-renamable display text.)*
...
- **Required fields:** `id`, `order`, `title`, `lessonCode` (short, e.g. `L03` — feeds the Activity ID scheme), `nextActivitySeq` (integer — the persisted, never-reused `seq` counter for this Lesson's Activities, §2.5; seeded to `1` at Lesson creation, advanced in the same transaction as each Activity mint, **never recomputed from surviving Activities**).
...
- **There is no `activities[]` field.** A Lesson's Activities are a query — `activities` filtered by `lessonId`, sorted by `Activity.order` — not an embedded array. Same reasoning as §2.4's `lessons[]`: `order` is the persisted source of truth for sequence, and a second one would only ever disagree with it.
...
- **Rules:** Content Planning fields are purely advisory — the page-range budget drives a *starting-page* default only (never an ending-page default) when the parent manually authors a new page-range Activity, filling gaps before extending past the budget's end; count targets drive a display-only "3 of 12" indicator. Neither is enforced or blocking; the parent can exceed, undershoot, or ignore either. Presets apply to manual single-Activity authoring only — bulk import is unaffected.
  - **`lessonCode` is unique within its Course**, and **freezes once any Activity exists under the Lesson.** Both are load-bearing, not cosmetic: with a literal `TPL` in segment 2, a template Activity ID carries no randomness at all, so its uniqueness rests entirely on `courseCode` + `lessonCode` + `seq`. Two Lessons sharing `L03` in one Course collide on the primary key; a renamed `lessonCode` strands old IDs and lets a later Lesson take the freed code and collide with them.
...
- **Rules:** pacing is per-instance, never shared. Changing it affects only *future* generation — it re-shapes how the **pending remainder** of the walk (§2.4) is distributed; Activities already recorded `sent` in the Generation Log (§2.10a) are never recalled or re-paced.

### 2.10 Generated Packet
- **Purpose:** the output of generation — a bounded slice of daily assignments for one child, reviewed by the parent and exported.
- **Generation unit: one child, one date range.** Generation is an interactive, single-session flow in three stages — **Propose → Review → Commit** — that produces exactly one Packet for that child and range. The Propose and Review stages live only in memory; nothing is written until Commit. Abandoning before Commit leaves storage untouched.

- **Propose** — the engine assembles a proposed set for the (child, range) from two parts:
  1. **Reproduce.** Every Generation Log decision (§2.10a) whose `assignedDate` falls in range is replayed: each `sent` Activity or Chore occurrence reappears on its recorded date — including one previously relocated or pulled forward — and each `dropped` Chore occurrence stays suppressed. Reproduced work is placed by the log, not re-derived by the walk; this is what makes re-generation of an already-committed range non-destructive.
  2. **Extend.** For each Child Course Instance, the **pending remainder** of the pacing walk (§2.4 — Lessons in `Lesson.order`, Activities in `Activity.order`, skipping any already `sent` or flagged `excludeFromGeneration`) is distributed into the in-range School days that still have pacing budget after step 1, in walk order, until content or range is exhausted. Each Chore's recurrence is expanded onto in-range dates that carry no prior decision, minting one deterministic per-occurrence ID (`CHR-{choreToken}-{YYYYMMDD}`) each. Family Events are filtered by date-range overlap and fanned out to each named child.
  Propose writes nothing.

- **Review** — the parent adjusts the proposed set before it ships. Five actions, none of which edits a Pacing Profile record:
  - **Relocate** an item to any date, including one outside the Instance's `daysOfWeek[]`.
  - **Exclude** an Activity permanently (`excludeFromGeneration`, Mgmt SRS 04) — a standing flag that also removes it from every future run.
  - **Defer** an Activity out of *this* run — it stays pending and re-proposes on a later run (no flag, no log row).
  - **Pull forward** an Activity the walk did not propose — the next pending one, or one out of sequence.
  - **Drop** a single Chore occurrence for its date — suppressed permanently for that one date, the Chore's recurrence untouched.
  The day's pacing budget is **advisory** here: the parent may leave a day heavier or lighter than `activitiesPerDay`/`minutesPerDay`, and the review overrides it without warning. Family Events are informational at this stage and are not individually adjustable.

- **Commit** — the only stage that writes. It records the reviewed set as decisions and exports:
  - one **Generation Log** row per `sent` item (Activity or Chore occurrence) on its final `assignedDate`; one `dropped` row per suppressed Chore occurrence; a **deferred** Activity gets no row (its absence is what keeps it pending); an **excluded** Activity persists its `excludeFromGeneration` flag and gets no row;
  - the exported **Packet** (sent items only) in the §4.1 shape;
  - **no Instance field is written** — there is no cursor to advance.

- **Merge order (fixed):** within a day, School Activities first, then Chores, then Family Events as day-level markers. This is the **Packet's array order**. `blockHint` does not affect it — but it is no longer "unused for ordering": on the child side it is the **outer** grouping axis of the Daily Plan (§3.4), with the School-then-Chores split nested *inside* each block. The Packet's merge order survives as the child's default *within-group* order (packet receipt order).
- **Empty-source rule:** a valid packet generates whenever *any* source has committed content for the range — a child with chores but no active instances still gets a packet.
- **Idempotency:** re-generating an already-decided range reproduces the identical Packet and rewrites its log rows identically; it never double-assigns, and never re-proposes an item already recorded `sent`, `dropped`, or `excludeFromGeneration`. Extending the range walks the pending remainder only across the newly-covered dates.

### 2.10a Generation Log
- **Purpose:** the authoritative record of every generation decision for a child — what was **sent**, on which date, and what was deliberately **dropped**. It plays two roles: the reporting source Master Reporting's Activity/Chore Roster reads, **and** the completeness record the generator itself consults to know what has already gone out and what is still pending. With no pacing cursor (§2.4), the log *is* the pointer.
- **Relationships:** one row per decided `(child, item)`. An Activity appears once — on its final `assignedDate`. A Chore occurrence appears once per date (the date is inside its `id`). Resolves to one Child (always) and one Course Instance (Activity rows only).
- **Required fields:** `childId`, `itemId` (an Activity `id`, or a Chore occurrence `id` `CHR-{choreToken}-{YYYYMMDD}`), `assignedDate` (the date the item was placed on — the relocated date if it was moved in Review), `disposition` (`sent` | `dropped`), `generatedAt`.
- **Optional fields:** `instanceId` (Activity rows only — absent for Chores, which have no Instance).
- **Identity:** one row per `(childId, itemId)`. Relocating an item, or re-committing an overlapping range, **updates that row in place** (new `assignedDate`/`generatedAt`) — never a second row. `sent` rows yield a Packet entry and a Roster row; `dropped` rows yield neither and exist solely to keep a suppressed Chore occurrence from being re-proposed on a later run.
- **Lifecycle:** written only at **Commit** (Management SRS Module 08), alongside the exported Packet. Read by the generator (to reproduce prior decisions and compute each Instance's pending remainder) and by Master Reporting's Roster. Written by no other module.
- **Rules:** flat, one-row-per-decision shape, matching the Activity Record's "one row per occurrence" convention (§3.6). A **deferred** or never-reached Activity has **no row** — that absence is the signal that keeps it pending, and is exactly how the next run re-proposes it.
...
- **Lifecycle:** proposed, reviewed, and committed on the Management side (§2.10); Commit writes the Generation Log and the Packet file together, before export — the one-way interchange makes a child-side trigger impossible → written to Drive → imported by the child app.
- **Rules:**
...
- **Generation Log (§2.10a)** — one row per generation decision (`sent`/`dropped`), keyed `(childId, itemId)`, written only at Commit; it is both Master Reporting's Roster source and the generator's own completeness record (there is no pacing cursor — §2.4).
