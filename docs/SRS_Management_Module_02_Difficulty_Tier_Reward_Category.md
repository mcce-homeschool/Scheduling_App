...
**FR-1 — Seed defaults on first launch.** The app initializes with **four** tiers: **Easy, Medium, Hard, Very Hard**, in that order, each paired with its own auto-minted Reward Category (`D01`/`R01` … `D04`/`R04`). This is a one-time seed, not a reset-able default — subsequent changes are the parent's own data.

*Very Hard is **seeded**, not merely parent-addable. §3's user story ("add a new difficulty level later, e.g. Very Hard") already made it addable via FR-2 with no document changes at all — but a parent-added tier's `categoryId` is only install-invariant **by coincidence**, and Architecture Evaluation §10 therefore forbids a theme from keying art to it. Seeding it is what makes `R04` mean Very Hard on **every** install, which is what makes it themeable. That is the entire difference between the two paths, and it is the reason to take this one.*
...
1. On first launch, exactly four tiers exist — Easy, Medium, Hard, Very Hard, in that order (`order` 0–3) — each with its own distinct `categoryId` already paired (`R01`–`R04`).
